...
Authoritative cache IO is C++ while the Python fallback keeps the same on-disk/on-blob schema for compatibility. Dict-shaped runtime/edit helpers remain part of the current implementation.

The current interactive paint tool also uses a runtime-only overlay for normal append paint and normal `eraseSpace == 0` erase.

- that overlay is not a serialized schema section and does not change schema version `2`
- it exists only to present visible authored state immediately without repacking the durable cache blob on every normal release
- visible authored state is therefore the durable baseline plus runtime overlay until an authoritative write/flush path persists the result
...
- schema version `2` adds authored color persistence for scatter paint output compatibility across compiled and Python-backed output paths
...
- partial rewrite optimization is not part of the current delivered contract, but multi-chunk generation and validation are part of it
...
This keeps barycentric attachment primary while preserving fallback data for diagnostics, reprojection, and current recovery behavior.
